...
keymap.set("i", "ii", "<Esc>", {noremap = true, silent = true, desc = "which_key_ignore"})
keymap.set("v", "ii", "<Esc>", {noremap = true, silent = true, desc = "which_key_ignore"})
keymap.set("c", "ii", "<C-c>", {noremap = true, silent = true, desc = "which_key_ignore"})
...
wk.add(
    {
        ...
        {
            "<leader>nd",
            "<cmd>Neotree toggle current reveal_force_cwd left<cr>",
            desc = "[N]eotree File [D]rawer",
            remap = false
        },
...
--     local black_path = home .. '/.pyenv/versions/nvim/bin/black'
-- 
--     if filetype == 'python' then
--         -- Use Black for Python files
--         vim.cmd('!' .. black_path .. ' -q ' .. vim.fn.expand('%'))
--         vim.cmd('edit!')
--         vim.notify('Formatted with Black', vim.log.levels.INFO)
--     else
--         -- Fallback to LSP for all other filetypes
...
-- => pylsp
------------------------------------------------------------------
-- Pylsp Configuration to Disable Internal Linters (e.g., pycodestyle, pyflakes)
...
--                 ruff = { enabled = false }, -- Set this to false if using standalone Ruff LSP
--                 isort = { enabled = true },
--                 black = { enabled = true },
--             },
--         },
...
-- => Ruff LSP
------------------------------------------------------------------
-- -- local ruff_path = require('mason-registry').get_package('ruff-lsp').bin_path .. '/ruff'
-- local ruff_path = vim.env.HOME .. '/.local/share/nvim/mason/bin/ruff'
-- -- 
-- vim.lsp.config('ruff', {
--     -- cmd = { 'ruff', 'server' },
...
--         local path_to_search = (type(fname) == 'string' and fname) or vim.api.nvim_buf_get_name(0)
--         if path_to_search == '' then
--             return vim.fn.getcwd()
--         end
--         local root_marker = vim.fs.find({'pyproject.toml', '.git'}, { upward = true, stop = vim.env.HOME, path = path_to_search })[1]
--         return root_marker and vim.fs.dirname(root_marker) or vim.fn.getcwd()
--     end,
-- 
--     on_attach = function(client, bufnr)
--         if client.server_capabilities.documentFormattingProvider then
--             vim.api.nvim_create_autocmd('BufWritePre', {
--                 group = vim.api.nvim_create_augroup('LspFormatting.ruff', { clear = true }),
--                 buffer = bufnr,
--                 callback = function()
...
------------------------------------------------------------------
-- require('yapf').setup {
--   command = vim.env.HOME .. '/.pyenv/versions/nvim/bin/yapf',
